...
def fetch_transcript_with_retry(yt_api: YouTubeTranscriptApi, 
                                video_id: str, 
                                max_retries: int = None) -> Optional[list]:
    """
        ...
        try:
            ...
            if attempt == max_retries - 1:
                ...
                try:
                    ...
                    logger.error(f"  ✗ No transcript available after {max_retries} attempts")
                    return None
            
            # Exponential backoff: 1s, 2s, 4s
            wait_time = 2 ** attempt
            logger.warning(f"  Retry {attempt + 1}/{max_retries} after {wait_time}s: {str(e)[:50]}")
...
def initialize_api() -> Optional[YouTubeTranscriptApi]:
    ...
    if Config.USE_PROXY:
        ...
        try:
            yt_api = YouTubeTranscriptApi(
                proxy_config=WebshareProxyConfig(
                    ...
                    proxy_password=Config.PROXY_PASS
                )
            )
        ...
        logger.warning("⚠️  Note: Without proxy, you may hit rate limits on large playlists")
...
def process_video(yt_api: YouTubeTranscriptApi, 
                 video, 
                 index: int, 
                 total: int, 
...
def main():
    ...
    for i, video in enumerate(playlist.videos, 1):
        success = process_video(yt_api, video, i, len(playlist.videos), Config.OUTPUT_FOLDER)
